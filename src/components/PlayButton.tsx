...
const PlayButton = ({ cacheKey, text, label = "듣기", className = "" }: PlayButtonProps) => {
  ...
  if (loading) {
    return (
      <div className={"inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[0.6875rem] font-medium bg-primary/10 text-primary " + className}>
  ...
  return (
    <button
      onClick={onMain}
      className={"inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[0.6875rem] font-medium bg-primary/10 text-primary active:bg-primary/20 " + className}
    >
      <Play size={14} /> {label}
    </button>
  );
