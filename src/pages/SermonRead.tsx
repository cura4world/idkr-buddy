...
const SermonRead = () => {
  ...
  // ---------- 본문에서 고른 표현 ----------
  // 안드로이드 기본 텍스트 선택을 그대로 씁니다 (다른 읽기 화면과 공용 모듈을 씁니다).
  ...
  const openPhrasePopup = () => {
    ...
    if (!s.phrase) return;
    lookupText(s.phrase, s.sentence);
  };
    ...
    <div className={"min-h-screen w-full " + widthClass + " mx-auto overflow-x-clip bg-background"}>
      ...
      {inkMode ? (
        ...
        >
          ...
          >
            {barHidden ? <ChevronDown size={18} /> : <ChevronUp size={18} />}
          </button>
        </div>
      ) : null}
      ...
      <PhraseFindBar
        phrase={selPhrase}
        onFind={openPhrasePopup}
        hidden={!!popupWord || tocOpen || inkMode}
        widthClass={widthClass}
      />

      {!loading && !error && rec && !inkMode && !popupWord && !tocOpen && !selPhrase ? (
