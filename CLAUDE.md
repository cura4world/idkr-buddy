...
3. **편집은 Edit / Write 도구로만 한다. Bash heredoc(`<<'EOF'`)·sed 로 코드를 쓰지 않는다.** 스크립트가 꼭 필요하면 그 스크립트도 Write 도구로 만들어 실행하고, 확인 뒤 저장소에 남기지 않는다. — heredoc이 백슬래시를 한 겹 먹어 `new RegExp("\\s+")`가 `\s+`로 기록된 사고가 있었다. esbuild·tsc를 모두 통과한 뒤 실행 시점에만 발각됐다. 백슬래시가 들어가는 코드는 무조건 Edit/Write 도구로 쓸 것.
4. 검증 명령을 모두 통과시킨다.
5. 파일별로 개별 커밋한다.
...
- **적응형 아이콘 배경색을 투명(`#00000000`)으로 두지 말 것** — 삼성 런처가 이 값을 **검정으로**
  칠한다. 전경이 타일을 거의 덮으면 가장자리에 검은 곡선으로, 덜 덮으면 아이콘 전체가 검게
  나온다. 배경은 항상 불투명 색(`#37C0D2`)으로 두고, 여백이 필요하면 전경 이미지 안에서 준다
...
**윈도우에서 실행할 때**: `/dev/null` 이 상대경로로 해석돼 저장소 안에 `dev\null` 파일이
생긴다. 커밋에 딸려 들어갈 수 있으므로 출력 경로를 저장소 밖으로 돌린다.

```powershell
npx esbuild <파일> --loader:.tsx=tsx --outfile=$env:TEMP\esbuild-check.js
```
...
- **기대값을 먼저 정하고 대조한다.** 명령을 돌려보고 결과를 해석하는 것이 아니라,
  ...
  - **`grep -c` 는 등장 횟수가 아니라 줄 수를 센다.** 한 줄에 같은 이름이 세 번 나오면 1로
    ...
    윈도우에는 `grep -o`·`wc -l` 이 없다. 줄 수는 `Select-String -SimpleMatch -CaseSensitive`,
    등장 횟수는 파이썬으로 센다 (한글이 섞이면 **PowerShell 콘솔이 깨뜨리므로 파이썬이 안전**)
  ...
  - **`grep` 이 대소문자를 구분하는지 먼저 확인한다.** 윈도우 PowerShell 의 `grep` 은
    `Select-String` 별칭이라 **기본이 대소문자를 무시**한다. `penDbg` 를 세었더니
    `setPenDbg` 까지 잡혀 기대값 2 에 실측 3 이 나온 적이 있다.
    `Select-String -CaseSensitive` 나 `findstr /C:` 를 쓰고, **어느 방식으로 셌는지 보고에 적는다**
  - **기대값이 어긋나면 지시가 틀렸을 가능성을 먼저 본다.** 프롬프트의 기대값은 사람이 손으로
    센 것이라 부분 문자열 겹침 때문에 자주 어긋난다 (`dispatchGenericMotionEvent` 안에
    `MotionEvent` 가 들어 있어 한 줄이 더 잡힌 전례). 순서는 이렇다 —
    ① `git diff` 로 편집 자체가 지시대로인지 증명 → ② 어긋난 숫자의 정체를 특정
    (부분 문자열 겹침인지, 대소문자인지, 줄 수/등장 횟수인지) → ③ 편집이 정확하다면
    **이유를 적고 진행하되 그 판단을 보고에 남긴다.** 편집이 틀린 것이면 커밋하지 말고 멈춘다
...
**7. 웹에서 아무리 해도 안 잡히는 입력은, 네이티브에서 가로챌 수 있는지 본다.**
이 앱은 우리가 만든 WebView 껍데기 안에서 돈다. 웹뷰가 어떤 입력을 웹 페이지에 넘기지 않아도,
**Activity 의 `dispatchTouchEvent` / `dispatchGenericMotionEvent` 는 뷰 계층보다 먼저 불리므로
그 이벤트를 받는다.** 웹에서 막히면 거기에 통로가 남아 있다.

(실제 사례: S펜 옆 버튼. `buttons`·호버·키 이벤트 어디에도 오지 않았고, `contextmenu` 는
오지만 롱프레스가 원인이었다 — 도착 시각이 펜이 닿은 지 4.3초 뒤였다. 결국 버튼을 누른 펜
터치는 웹뷰가 통째로 삼키고 있었다(필기면·문서 양쪽 `pointerdown` 이 0). 네이티브에서
`BUTTON_STYLUS_PRIMARY` 를 읽어 `window.__penBtn` 으로 알리고, 좌표는 `window.__penErase`
로 넘겨 해결했다.)

- 네이티브 → 웹 좌표는 **웹뷰 크기 대비 비율(0~1)로 넘긴다.** px 로 넘기면 화면 배율·해상도·
  초기 확대율을 모두 맞춰야 한다. 비율이면 웹에서 `window.innerWidth` 만 곱하면 된다
- 가로챈 제스처는 **시작한 획이 끝날 때까지 계속 가로챈다**(플래그 하나). 중간에 놓으면
  손을 떼는 순간을 웹이 못 받아 저장이 걸리지 않는다
- 네이티브 코드는 **APK 재빌드 + 두 기기 재설치**가 필요하다. 그래서 넘어가기 전에
  "정말 웹으로는 안 오는가"를 숫자로 확정하고 간다(6번의 진단 장치)
