...
## 9. 단어장은 왜 "책"과 "노트"로 나뉘었나 (2026-08)

공용 단어장을 주제별 36권 3,067단어로 정비한 뒤, 단어장의 역할을 둘로 갈랐습니다.

| | 무엇 | 어디 | 편집 |
|---|---|---|---|
| **책** | 시드에서 온 공용 단어장 36권 | 단어장 폴더 `/wordbooks` | 읽기 전용 |
| **노트** | 사용자가 담고 만든 단어장 | 내 단어장 `/category/my-wordbook` 안 | 자유 |

### 공용 단어장을 읽기 전용으로 만든 이유

제약을 새로 건 것이 아니라 **원래 그랬던 동작을 화면에 정직하게 반영**한 것입니다.
`initSharedCategories`가 배포마다 `saveWords([...seedWords, ...existingWords])`로 공용 단어를
시드로 교체하므로, 이름을 바꿔도 단어를 지워도 다음 배포에서 되돌아갑니다.
즉 편집 기능이 있어도 **헛돌기만 하고 사용자를 헷갈리게 할 뿐**이었습니다.
...
`my-wordbook`과 사용자가 만든 단어장은 `isShared`가 없어 `true`,
시드에서 온 공용·개인 폴더(`data/private/`) 단어장은 `false`가 됩니다.
`editable`이 거짓이면 상단 버튼 줄, 단어 선택, 왼쪽 스와이프(삭제), 드래그 순서 이동을 모두 막습니다.
**오른쪽 스와이프(복사)와 롱프레스 단어정보는 남깁니다** — 읽기에 유용하고 데이터를 바꾸지 않습니다.

`restoreSharedCategories`(복구 버튼)는 제거했습니다. 복구는 `initSharedCategories`의
자동 동기화가 이미 담당하므로 버튼이 할 일이 없었습니다.

### 내 단어장의 흐름

```
사전·이야기에서 담기 → 내 단어장에 쌓임
   → 100개에서 묶기 권유 배너
   → 이름 붙여 묶기 (archiveMyWordbook)
   → 보관 목록에 최신이 위로 쌓임 / 내 단어장은 비워져 다시 시작
```

- 배너는 `✕`로 닫으면 그때의 단어 수를 `my-archive-nudge-at`에 적어 두고, **50개 더 모이면** 다시 뜹니다.
- 보관 단어장은 톱니에서 이름 변경·삭제만 됩니다. 순서 이동은 없습니다 — 최신순 고정입니다.
- 잘못 들어간 단어는 골라서 `moveWordsToCategory`로 다른 단어장에 옮깁니다.
  대상에 같은 표제어가 이미 있으면 건너뛰고 원래 자리에 둡니다(대소문자 무시).
- 단어가 많아 스크롤이 길어지므로 `단어 N개` / `보관한 단어장 N권` 소제목을 눌러 각각 접을 수 있습니다.
  접힘 상태는 단어장별로 localStorage에 남습니다.

### 표시 순서 = 저장 순서

새 단어와 새 단어장은 **배열 앞에 삽입**합니다. 화면에서 역순으로 정렬하지 않습니다.

- `addWord`: 내 단어장 단어는 그 구간 맨 앞에
- `addCategory`: `my-wordbook` 바로 다음에
- `moveWordsToCategory`: 대상 단어장 구간 맨 앞에

이렇게 해야 나중에 드래그로 순서를 바꿨을 때 **화면 순서와 저장 순서가 어긋나지 않습니다.**
역순 정렬로 최신을 위에 보이게 하면, 드래그로 옮긴 결과가 다음 진입 때 뒤집혀 보입니다.
...
| 단어장 폴더 화면 재디자인 | 완료 | `rounded-2xl` 행 리스트 + 번호 범위 그룹 소제목. 읽기 전용이 되어 드래그 정렬은 제거 (§9) |
| 단어장 책/노트 분리 | 완료 | 3단계로 마무리 (§9) |
