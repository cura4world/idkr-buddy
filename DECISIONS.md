...
### heredoc이 백슬래시를 한 겹 먹는다 (2026-08)
Claude Code의 Bash heredoc으로 코드를 쓰면 백슬래시가 한 겹 사라집니다. `new RegExp("\\s+")`가 `\s+`로 기록됐고, **esbuild와 tsc를 모두 통과한 뒤 실행 시점에만** 발각됐습니다.
→ 치환 스크립트는 Write 도구로 파일을 만들어 실행합니다. 백슬래시가 들어가는 코드는 heredoc으로 쓰지 않습니다.

### 실패 안내문이 데이터로 저장된 사고 (2026-08)
단어 팝업에서 Gemini 조회가 실패하면 `setPopupMeaning("뜻을 불러오지 못했어요…")`로 **뜻을 담는 변수에 안내문을 넣었습니다.** 저장 가드는 빈 문자열만 막았기 때문에, 그 순간 "담기"를 누르면 안내문이 단어의 뜻으로 저장됐습니다. 그리고 퀴즈·게임은 단어의 뜻을 그대로 보기로 쓰므로, 몇 주 뒤 엉뚱한 화면에서 튀어나왔습니다.
→ **실패 상태를 정상 값과 같은 변수에 담지 않습니다.** 하류의 어떤 코드도 그것이 실패인지 알 수 없습니다.
→ 대응은 두 겹입니다. `store.ts`의 `addWordIfAbsent`가 `isFailedMeaning`으로 저장 자체를 막고, 8개 팝업 화면은 실패 시 담기 버튼을 비활성화합니다. 예전에 저장된 것은 `store.ts`가 **앱 시작 시 자동으로 지우고**(`purgeFailedMeaningWords`), 지운 단어 이름을 메인 화면이 토스트로 한 번 알립니다. 이 청소 코드는 의도된 것이니 지우지 마세요.
...
---
## 10. Medali — 왜 두 축인가 (2026-08)

명찰은 불꽃(Api)과 별(Bintang) 두 개입니다. **재는 것이 다르기 때문에 둘입니다.**

| | 무엇을 재나 | 리셋 |
|---|---|---|
| **Api (불꽃)** | 이번 주에 얼마나 했나 — 읽기·조회·저장·회화·탐험·게임 전부 | 주 단위 (월요일 자연 0) |
| **Bintang (별)** | 실제로 아는 단어가 몇 개인가 | 없음 (평생 누적) |

그래서 사전을 아무리 찾아봐도 별은 오르지 않습니다. 그 노력은 Api에 쌓입니다. 별은 **검증된 것만** 셉니다.

### 확정 판정 — 정수가 아니라 증거 점수

정답 1회를 1로 세지 않고 **소스별 가중치를 누적**합니다. 찍어서 맞을 확률이 게임마다 다르기 때문입니다.

| 소스 | 정답 증거 | 이유 |
|---|---|---|
| 퀴즈 · Eja · Susun · Tangkap | +1.0 | 우연히 맞기 어려움 |
| 스피드 OX | +0.5 | 2지선다라 50% |
| 짝맞추기 Cocokkan | +0.5 | 소거법이 가능 |

- **확정 조건**: 누적 증거 ≥ 2.0 **그리고** 첫 정답에서 3일 이상 경과
- 강한 소스 2회면 확정, OX만으로는 4회 필요합니다. "50%를 네 번 연속"이면 그것대로 증거라고 봤습니다.
- **오답은 소스와 무관하게 같은 강도**입니다. 이 비대칭은 의도입니다 — 맞힌 것은 우연일 수 있지만 틀린 것은 어디서 틀려도 확실한 신호입니다.

### 오답 사다리 — 한 번에 떨어뜨리지 않는다

```
confirmed ──오답──> recheck ──오답──> monitoring ──오답──> pending (별 -1)
confirmed ──60일 경과──> recheck
recheck / monitoring ──복귀 증거 1.0──> confirmed (60일 시계 리셋)
```

- **`recheck`와 `monitoring`도 별 개수에 포함됩니다.** 숫자가 출렁이면 동기가 꺾이기 때문에, 진짜 강등은 마지막 한 칸에서만 일어납니다.
- `monitoring`은 "한 번 더 기회"입니다. recheck에서 틀렸다고 즉시 깎지 않습니다.
- **recheck 상태에서 오답이 별을 줄이지 않는 것은 버그가 아닙니다.** 고치지 마세요.
- 60일 만료는 **배치 작업이 없습니다.** 주간 리셋과 같은 원리로, `effectiveStatus(rec, now)`가 읽을 때 계산합니다. `confirmedAt`이 없는 옛 레코드는 만료시키지 않습니다.

### 게임은 검증 통로다

- 게임 5종(짝맞추기·OX·문장조립·철자채우기·단어받기)이 **하루 15점 상한을 공유**합니다. 많이 한다고 Api가 무한정 오르지 않습니다.
- `gamePool`의 출제 순서: `monitoring` → `recheck` → **확정 직전 단어**(증거가 1 남고 3일이 지난 것) → 찾아본 단어 → 단어장 → 시드.
  확정 직전 단어를 앞줄에 두는 이유는, 단어가 늘어날수록 특정 단어가 게임에 뽑힐 확률이 떨어져 **실력과 별이 벌어지기** 때문입니다.
- `drawPool`은 확정 단어 몫의 절반을 미확정 쪽으로 옮깁니다. 미확정 후보가 부족하면 옮기지 않으므로 초기 상태에서는 예전과 동일하게 동작합니다.
- **게임 화면 5개는 출제 비율을 모릅니다.** 조정은 `gamePool.ts` 안에서만 합니다.

### 저장 구조

IndexedDB `kata-medali` 3종 — 일별 로그(`DailyLog`) · 단어 기록(`WordRecord`) · 게임 판 기록(`rounds`, 500판 FIFO).
localStorage `medali-cache`는 첫 페인트에 명찰 색·숫자를 즉시 그리기 위한 사본입니다(깜빡임 방지).
주간 점수는 저장하지 않고 **주차 키로 로그를 매번 합산**합니다. 그래서 월요일에 자연히 0이 되고, 리셋 배치가 필요 없습니다.
부부 비교(2단계)를 위해 `getSummary()`가 미리 준비돼 있습니다.
...
| Medali 커트라인·점수 보정 | 대기 | 실사용 뒤 일별 로그를 근거로 한 번에 조정 (지금은 추정치) |
| 게임별 정답률 분석 | 대기 | `rounds` 스토어에 500판까지 쌓이는 중. 이 데이터로 게임을 정교화하고 신규 게임을 검토 |
| 부부 비교 2단계 | 보류 | Cloudflare KV. `getSummary()`는 준비됨 |
| Medali 설계 노트 md | 검토 | 나루브 등 다른 앱 이식용 |
